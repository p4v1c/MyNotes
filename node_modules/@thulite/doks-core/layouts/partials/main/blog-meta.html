  ...
  <p>
    <small>
      ...
      {{- range $index, $contributor := .Params.contributors -}}
        ...
        {{- with $.Site.GetPage "taxonomyTerm" (printf "contributors/%s" (urlize $contributor)) -}}
          {{ if .Params.avatar -}}
          ...
          {{ end }}
        {{- end -}}
        <a class="stretched-link position-relative" href="{{ "/contributors/" | relLangURL }}{{ $contributor | urlize }}/">{{ $contributor }}</a>
      {{- end }}
  ...
  <p>
    <small>
      ...
      {{- with .Params.categories -}}
        ...
        {{- end }}
      {{- end }}
      {{- /* If no contributors, skip this section */ -}}
      <span class="stretched-link position-relative reading-time text-nowrap" title="{{ i18n "reading_time" }}">{{/* trim subsequent whitespace */ -}}
        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-clock" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
          <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
          <path d="M12 7v5l3 3"></path>
